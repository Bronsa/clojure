...
(import-if
 ...
 (java.util.concurrent ForkJoinTask ForkJoinPool)
 (jsr166y              ForkJoinTask ForkJoinPool))
...
(defn- foldseq [num-tasks coll chunk-size combinef reducef]
  (fjinvoke (fn []
    (let [chunk-task (fn [chunk] (fjfork (fjtask #(reduce reducef (combinef) chunk))))]
      (reduce combinef
        (core/map #(fjjoin (first (doall %)))
          (partition-all num-tasks 1 (core/map chunk-task (partition-all chunk-size coll)))))))))
